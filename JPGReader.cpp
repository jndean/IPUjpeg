...
JPGReader::JPGReader(poplar::Target &ipuTarget)
    : m_ready_to_decode(false),
      ...
      m_num_bufbits(0),
      m_ipuGraph(ipuTarget) {
  for (int i = 0; i < 3; ++i) {
    m_channels[i].pixels = nullptr;
  }

  // Create the output pixel tensors //
  poplar::Tensor out_pixels = m_ipuGraph.addVariable(poplar::UNSIGNED_CHAR, {MAX_PIXELS_Y, MAX_PIXELS_X, 3}, "out_pixels");

  for (unsigned long y = 0; y < JPGReader::TILES_Y; y++) {
    for (unsigned long x = 0; x < JPGReader::TILES_X; x++) {
      poplar::Tensor patch = out_pixels.slice(
        {x * (PIXELS_PER_TILE_X    ), y * (PIXELS_PER_TILE_Y    )},
        {x * (PIXELS_PER_TILE_X + 1), y * (PIXELS_PER_TILE_Y + 1)}
      );
    }
  }
...
void JPGReader::read(const char *filename) {
  ...
  // Check Magics //
  if ((m_buf[0] != 0xFF) || (m_buf[1] != 0xD8) || (m_buf[m_size - 2] != 0xFF) || (m_buf[m_size - 1] != 0xD9)) goto FAILURE;
...
void JPGReader::flush() {
  SAFEDELETE(m_buf);
  SAFEDELETE(m_pixels);
  for (ColourChannel &c : m_channels) {
    SAFEDELETE(c.pixels);
  }
  m_ready_to_decode = false;
  m_error = NO_ERROR;
...
void JPGReader::decodeSOF() {
  ...
  if (m_width > JPGReader::MAX_PIXELS_X || m_height > JPGReader::MAX_PIXELS_Y) {
    throw std::runtime_error("Output reolution too big");
  }
